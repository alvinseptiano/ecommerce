<header x-data="{
    ...
    }" @cart-change.window="cartItemsCount = $event.detail.count" class="flex justify-between shadow-md ">
    <div class="flex items-center space-x-2 h-14">
        <a href="{{ route('home') }}" class="py-navbar-item pl-5 mt-4"> <img class="w-20 h-full"
    ...
    <div class="block fixed z-10 top-0 bottom-0 height h-full w-[220px] transition-all md:hidden" :class="mobileMenuOpen ? 'left-0' : '-left-[220px]'">
        <ul>
            <liv>
                <div class="relative flex items-center justify-between py-2 px-3" x-data="{
            ...
            <li>
                <a href="{{ route('cart.index') }}" class="relative flex items-center justify-between py-2 px-3">
                    ...
                    <small x-show="cartItemsCount" x-transition x-text="cartItemsCount" x-cloak
                        class="py-[2px] px-[8px] rounded-full bg-yellow"></small>
                    <!--/ Cart Items Counter -->
                </a>
            </li>
            @if (!Auth::guest())
                    ...
                    <ul x-show="open" x-transition class="z-10 right-0 bg-slate-800 py-2">
                        ...
                        <li class="">
                            ...
                            <form method="POST" action="{{ route('logout') }}">
                                ...
                                <a href="{{ route('logout') }}" class="flex items-center px-3 py-2 " onclick="event.preventDefault();
    ...
    <nav class="hidden md:block">
        <ul class="grid grid-flow-col items-center">
            <li>
                <a href="{{ route('cart.index') }}"
                    ...
                    <small x-show="cartItemsCount" x-transition x-cloak x-text="cartItemsCount"
                        class="absolute z-[100] top-4 -right-3 py-[2px] px-[8px] rounded-full bg-error"></small>
                </a>
            </li>
            @if (!Auth::guest())
                <li x-data="{open: false}" class="relative">
                    ...
                    <ul @click.outside="open = false" x-show="open" x-transition x-cloak
                        class="rounded-lg absolute z-10 right-0 bg-base-100 py-2 w-48 text-accent-content">
                        <li>
                            <a href="{{ route('profile') }}" class="flex px-3 py-2 ">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor" stroke-width="2">
                        ...
                        <li>
                            <form method="POST" action="{{ route('logout') }}">
                                ...
                                <a href="{{ route('logout') }}" class="flex px-3 py-2 " onclick="event.preventDefault();
                                                                this.closest('form').submit();">
            ...
            @else
                <li>
                    <a href="{{ route('login') }}" class="flex items-center py-navbar-item px-navbar-item ">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1" />
