  ...
  <div>
    ...
    <div class="mt-1 flex rounded-md" :class="type === 'checkbox' ? 'items-center' : 'shadow-sm'">
      ...
      <template v-else-if="type === 'richtext'">
        <ckeditor :editor="editor" :required="required" :model-value="props.modelValue" @input="onChange"
          :class="inputClasses" :config="editorConfig"></ckeditor>
      </template>
...
<style scoped>
.parent :deep(.ck-editor) {
  width: 100%;
}

.parent :deep(.ck-content) {
  width: 100%;
}
