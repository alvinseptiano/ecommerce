	...
	<div class="bg-white rounded-lg shadow animate-fade-in-down">
		...
		<form v-if="!loading" @submit.prevent="onSubmit">
			<div class="grid grid-cols-3">
				<div class="col-span-2 px-5 pt-5 pb-4">
					<CustomInput class="mb-2 max-w-full" v-model="product.title" label="Product Title"
						:errors="errors['title']" />
					<CustomInput type="richtext" class="mb-2 max-w-full" v-model="product.description" label="Description"
			...
			<footer class="bg-gray-50 rounded-b-lg px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse justify-start">
				...
				<button type="submit" @click="onSubmit($event, true)" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 text-base font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm
                          text-white bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500">
					Save & Close
				</button>
